...
def get_session_summaries():
    ...
    return jsonify(
        {
            'files': [{'name': p.name, 'content': p.read_text()} for p in summaries[:5]],
            'error': None,
        }
    )
    ...
    try:
        result = subprocess.run(
            [
                ...
                'list',
                '--json',
                'number,title,labels,state,createdAt',
                '--state',
                'open',
                '-L',
                '50',
...
def get_pm_sessions():
    ...
    if not pm_sessions_file.exists():
        return jsonify({'version': '1.0.0', 'last_cleanup': None, 'sessions': [], 'error': None})
    ...
    try:
        ...
        return jsonify({'version': '1.0.0', 'last_cleanup': None, 'sessions': [], 'error': str(e)})
