...
class TestEventEmission:
    ...
    def test_events_jsonl_format(self, memory_dir_with_claude_md: Path, monkeypatch):
        ...
        entry = log_session.SessionEntry(
            timestamp=datetime(2026, 2, 2, 10, 30, 0, tzinfo=timezone.utc),
            task="Test task",
            outcome="SUCCESS",
            files=["file.py"],
            decisions=[("d1", "r1", "a1")],
...
class TestQuickMode:
    ...
    def test_quick_mode_with_task_id(self, mock_git_unavailable):
        ...
        entry = log_session.gather_quick(
            task="Quick task", outcome="SUCCESS", task_id="TASK-42"
        )
...
class TestTaskIdDetection:
    ...
    def test_detect_task_id_from_workflow_state(
...
class TestMemoryDirDetection:
    ...
    def test_get_memory_dir_creates_if_needed(
