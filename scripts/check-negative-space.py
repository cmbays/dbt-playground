...
def find_matches(
    question: str, decisions: list[dict], threshold: float = 0.4
...
def list_all_decisions(decisions: list[dict]) -> None:
    ...
    for decision in decisions:
        ...
        table.add_row(
            decision.get('id', '?'),
            decision.get('question', '?')[:50] + '...'
            if len(decision.get('question', '')) > 50
            else decision.get('question', '?'),
  ...
  uv run scripts/check-negative-space.py --check-triggers
    ...
    if args.question:
        ...
        if not matches:
            ...
            console.print(
                '[dim]This question has not been previously addressed in negative space.[/dim]'
            )
            return 0
