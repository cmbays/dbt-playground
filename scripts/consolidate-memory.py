...
STOP_WORDS = {
    ...
    'and', 'but', 'or', 'if', 'because', 'while', 'this', 'that', 'it', 'its',
    'not', 'no', 'yes', 'also', 'just', 'only', 'very', 'more', 'most', 'some',
    'can', 'need', 'use', 'using', 'used', 'none', 'documented', 'nothing', 'noted'
}
...
class PatternGroup:
    ...
    def dates(self) -> list[date]:
        return sorted(set(item.entry_date for item in self.items))
...
def get_memory_dir() -> Path:
    ...
    for parent in [Path.cwd()] + list(Path.cwd().parents):
        if (parent / 'CLAUDE.md').exists():
            ...
            return mem
    raise FileNotFoundError("Not in a dbt-playground project")
...
def parse_log_file(path: Path) -> list[dict]:
    ...
    for section in sections[1:]:
        ...
        if learnings_match:
            ...
            entry['learnings'] = [l for l in learnings if l not in ('None documented', 'None')]

        # Parse decisions
        decisions_match = re.search(r'\*\*Key Decisions\*\*:\n((?:- .+\n?)+)', section)
        if decisions_match:
            decisions = re.findall(r'- (.+)', decisions_match.group(1))
            entry['decisions'] = [d for d in decisions if d not in ('None documented', 'None')]

        # Parse improvements
        improve_match = re.search(r'\*\*Would Do Differently\*\*:\n((?:- .+\n?)+)', section)
        if improve_match:
            improvements = re.findall(r'- (.+)', improve_match.group(1))
            entry['improvements'] = [i for i in improvements if i not in ('Nothing noted', 'Nothing')]
...
def score_pattern(group: PatternGroup, today: date) -> float:
    ...
    unique_tasks = len(set(item.source_task for item in group.items if item.source_task))
...
def determine_status(group: PatternGroup) -> str:
    ...
    if group.score >= 0.7:
        ...
        return 'CANDIDATE'
    elif group.count >= 2 and len(group.dates) >= 2:
        return 'CANDIDATE'
    elif group.count >= 2:
        return 'REVIEW'
    return 'REJECTED'
...
def generate_memory_index(result: dict, memory_dir: Path) -> str:
...
def main():
    ...
    try:
        ...
        if not args.dry_run:
            ...
            index_content = generate_memory_index(result, memory_dir)
